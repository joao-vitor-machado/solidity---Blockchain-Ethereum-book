...
contract StudentsContract {
    ...
    function getEnrolledStudentByAddress(address _studentAdress) public view returns (Student memory){

        return _enrolledStudents[_studentAdress];
    ...
    function isStudentValid(Student memory _student) private pure returns (bool) {
        return isStudentAgeValid(_student.studentAge) && isStudentNameValid(_student.studentName);
    }

    function enrollStudent(string memory _studentName, uint _studentAge) public {
        require(!isStudentValid(
            getEnrolledStudentByAddress(msg.sender)
        ), 
        "Student already enrolled"
        );

        Student memory student = Student(_studentName, _studentAge);
